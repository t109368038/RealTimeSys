...
def Range_Angle(data, mode=0, padding_size=None, windowing=True):
    ...
    #
    # window_3 = np.reshape(np.hanning(data.shape[1]), (data.shape[1], -1))
    # window_4 = np.reshape(np.hanning(data.shape[2]), (-1, data.shape[2]))
    # window5 = window_3 * window_4
    # channel2 = data.shape[1]
    # for i in range(channel2):
    #     data[i, :, :] = data[i, :, :] * window5.T
    # data = np.fft.fft2(data, s=[padding_size[0], padding_size[2]], axes=[0, 2])
    # rai_abs = np.fft.fftshift(np.abs(data), axes=2)
    # rai_abs = np.flip(rai_abs, axis=1)
    # return rai_abs
